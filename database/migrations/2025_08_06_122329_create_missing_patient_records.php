...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        foreach ($patientUsers as $user) {
            // Create patient record for each user with correct field mappings
            Patient::create([
                'user_id' => $user->id,
                'first_name' => $user->first_name,
                'last_name' => $user->last_name,
                'contact' => $user->phone_number, // Correct field name
                'email' => $user->email,
                'address' => $user->street_address, // Correct field name
                'date_of_birth' => $user->date_of_birth ?? '1990-01-01',
                'gender' => $user->gender ?? 'other',
                'blood_type' => 'O+', // Default blood type
                'emergency_contact' => 'Emergency Contact',
                'emergency_contact_phone' => $user->phone_number, // Correct field name
