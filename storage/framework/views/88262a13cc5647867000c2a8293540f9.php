...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="text-3xl font-bold text-blue-700 mb-2"><?php echo e($billings->total()); ?></div>
        ...
        <div class="text-3xl font-bold text-green-700 mb-2"><?php echo e($billings->filter(fn($b) => $b->status === 'paid')->count()); ?></div>
        ...
        <div class="text-3xl font-bold text-yellow-700 mb-2"><?php echo e($billings->filter(fn($b) => $b->status === 'unpaid')->count()); ?></div>
                    ...
                    <tr class="hover:bg-blue-50 transition">
                        <td class="py-3 px-4 flex items-center gap-3">
                            <span class="inline-flex items-center justify-center w-10 h-10 bg-blue-100 rounded-full text-blue-600 font-bold text-lg">
                                <?php echo e(strtoupper(substr($billing->patient->first_name ?? '-', 0, 1))); ?>

                            </span>
                            <div>
                                <div class="font-medium"><?php echo e($billing->patient->first_name ?? '-'); ?> <?php echo e($billing->patient->last_name ?? ''); ?></div>
                                <div class="text-xs text-gray-500"><?php echo e($billing->patient->email ?? ''); ?></div>
                            </div>
                        </td>
                        <td class="py-3 px-4">
                            <div class="font-medium"><?php echo e($billing->appointment ? $billing->appointment->appointment_date->format('M d, Y') : '-'); ?></div>
                            <div class="text-xs text-gray-500"><?php echo e($billing->appointment ? $billing->appointment->appointment_time->format('h:i A') : ''); ?></div>
                        </td>
                        <td class="py-3 px-4">
                            <div class="font-medium">₱<?php echo e(number_format($billing->consultation_fee ?? 0, 2)); ?></div>
                        </td>
                        <td class="py-3 px-4">
                            <div class="font-medium">₱<?php echo e(number_format($billing->medication_fee ?? 0, 2)); ?></div>
                        </td>
                        <td class="py-3 px-4">
                            <div class="font-medium">₱<?php echo e(number_format($billing->laboratory_fee ?? 0, 2)); ?></div>
                        </td>
                        <td class="py-3 px-4">
                            <div class="font-medium">₱<?php echo e(number_format($billing->other_fees ?? 0, 2)); ?></div>
                        </td>
                        <td class="py-3 px-4">
                            <div class="font-bold text-lg">₱<?php echo e(number_format($billing->total_amount ?? $billing->amount, 2)); ?></div>
                        </td>
                        <td class="py-3 px-4">
                            <?php
                            ...
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border <?php echo e($badge); ?>">
                                <i class="fas fa-circle mr-2 text-[6px]"></i> <?php echo e(ucfirst($billing->status)); ?>

                            </span>
                        </td>
                        <td class="py-3 px-4">
                            <div class="font-medium"><?php echo e($billing->payment_date ? \Carbon\Carbon::parse($billing->payment_date)->format('M d, Y') : '-'); ?></div>
                            <div class="text-xs text-gray-500"><?php echo e($billing->payment_date ? \Carbon\Carbon::parse($billing->payment_date)->format('h:i A') : ''); ?></div>
                        </td>
                        <td class="py-3 px-4">
                            <div class="flex gap-2">
                                <?php if($billing->status === 'unpaid'): ?>
                                    <a href="<?php echo e(route('staff.invoice.mark-as-paid', $billing->id)); ?>"
                                       class="btn btn-success btn-sm flex items-center gap-1 !w-24 justify-center"
                                       onclick="return confirm('Mark this invoice as paid?')">
                                        <i class="fas fa-check"></i> Mark Paid
                                    </a>
                                <?php endif; ?>
                                <a href="<?php echo e(route('staff.invoice.generate-pdf', $billing->id)); ?>"
                                   class="btn btn-primary btn-sm flex items-center gap-1 !w-24 justify-center">
                                    <i class="fas fa-file-pdf"></i> Download PDF
                                </a>
                                <a href="<?php echo e(route('staff.invoice.generate-pdf', [$billing->id, 'print' => 1])); ?>" target="_blank"
                                   class="btn btn-secondary btn-sm flex items-center gap-1 !w-24 justify-center">
                                    <i class="fas fa-print"></i> Print
                                </a>
                                <form action="<?php echo e(route('staff.invoice.destroy', $billing->id)); ?>" method="POST" class="inline delete-invoice-form">
                                    <?php echo csrf_field(); ?>
                                    ...
                                    <button type="submit" class="delete-invoice-btn btn btn-secondary btn-sm flex items-center gap-1 !w-24 justify-center !text-orange-700 !bg-orange-50 !border-orange-200 hover:!bg-orange-600 hover:!text-white"
                                            data-patient="<?php echo e($billing->patient->first_name ?? ''); ?> <?php echo e($billing->patient->last_name ?? ''); ?>"
                                            data-amount="<?php echo e(number_format($billing->total_amount ?? $billing->amount, 2)); ?>"
