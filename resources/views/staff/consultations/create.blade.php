...
<div class="consultations-content">
    ...
    <div class="flex justify-between items-center">
        ...
        <div class="flex gap-3">
            <a href="{{ route('staff.consultations.index') }}" class="btn btn-secondary flex items-center gap-2">
                            ...
                            <select name="appointment_id" id="appointment_id"
                                ...
                                @foreach($appointments ?? [] as $appointment)
                                    @if($appointment && is_object($appointment))
                                        <option value="{{ $appointment->id ?? '' }}" data-patient="{{ $appointment->patient_id ?? '' }}" data-doctor="{{ $appointment->doctor?->user_id ?? '' }}" data-date="{{ $appointment->appointment_date ?? '' }}" data-time="{{ $appointment->appointment_time ?? '' }}">
                                            {{ $appointment->patient?->user?->first_name ?? '' }} {{ $appointment->patient?->user?->last_name ?? '' }} -
                                            Dr. {{ $appointment->doctor?->user?->first_name ?? '' }} {{ $appointment->doctor?->user?->last_name ?? '' }} -
                                            {{ $appointment->appointment_date ? \Carbon\Carbon::parse($appointment->appointment_date)->format('M d, Y') : 'N/A' }} at {{ $appointment->appointment_time ?? 'N/A' }}
                                        </option>
                                    @endif
                                 @endforeach
                            </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function saveDraft() {
        ...
        fetch('{{ route("staff.consultations.store") }}', {
            method: 'POST',
            body: formData,
            headers: {
