...
@section('title', 'Profile - iWellCare')
            ...
            <div class="flex items-start gap-6 mb-6">
                ...
                <div class="flex-1">
                    <div class="flex items-center gap-3 mb-2">
                        <h2 class="text-2xl font-bold text-gray-800">{{ $user->first_name ?? 'Staff' }} {{ $user->last_name ?? '' }}</h2>
                        <span class="inline-flex items-center px-3 py-1 bg-blue-100 text-blue-700 text-xs font-medium rounded-full">
                            <i class="fas fa-shield-alt mr-1"></i> {{ ucfirst($user->role ?? 'staff') }}
                        </span>
                    </div>
                    <div class="text-gray-600 mb-1">{{ $user->email ?? 'staff@email.com' }}</div>
                    <div class="text-gray-500 text-sm">Username: {{ $user->username ?? 'staffuser' }}</div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-4">
                    <div class="flex justify-between items-center py-3 border-b border-gray-100">
                        ...
                        <span class="text-gray-800">{{ $user->first_name ?? 'Staff' }} {{ $user->last_name ?? '' }}</span>
                    </div>
                    <div class="flex justify-between items-center py-3 border-b border-gray-100">
                        <span class="text-gray-600 font-medium">Email Address</span>
                        <span class="text-gray-800">{{ $user->email ?? 'staff@email.com' }}</span>
                    </div>
                    <div class="flex justify-between items-center py-3 border-b border-gray-100">
                        <span class="text-gray-600 font-medium">Username</span>
                        <span class="text-gray-800">{{ $user->username ?? 'staffuser' }}</span>
                    </div>
                    <div class="flex justify-between items-center py-3 border-b border-gray-100">
                        <span class="text-gray-600 font-medium">Role</span>
                        <span class="text-gray-800 capitalize">{{ $user->role ?? 'staff' }}</span>
                    </div>
                </div>
                <div class="space-y-4">
                    <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    <div class="flex justify-between items-center py-3 border-b border-gray-100">
                        ...
                        <span class="text-gray-800">{{ $user->created_at ? $user->created_at->format('M d, Y') : 'N/A' }}</span>
                    </div>
                    <div class="flex justify-between items-center py-3 border-b border-gray-100">
                        <span class="text-gray-600 font-medium">Last Login</span>
                        <span class="text-gray-800">{{ $user->last_login_at ? \Carbon\Carbon::parse($user->last_login_at)->diffForHumans() : 'Never' }}</span>
                    </div>
                    <div class="flex justify-between items-center py-3 border-b border-gray-100">
                        <span class="text-gray-600 font-medium">Profile Updated</span>
                        <span class="text-gray-800">{{ $user->updated_at ? $user->updated_at->format('M d, Y') : 'N/A' }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card p-6">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                    ...
                    <span class="text-xs text-blue-600">{{ $user->last_login_at ? \Carbon\Carbon::parse($user->last_login_at)->diffForHumans() : 'Never' }}</span>
                </div>
                ...
                <div class="flex items-center justify-between p-3 bg-yellow-50 rounded-lg">
                    ...
                    <span class="text-xs text-yellow-600">{{ $user->created_at ? $user->created_at->format('M d') : 'N/A' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="card p-6 border border-red-200 bg-red-50">
            ...
            <div class="space-y-4">
                <div class="p-4 bg-white rounded-lg border border-red-200">
                    ...
                    <form action="{{ route('staff.profile.destroy') }}" method="POST" onsubmit="return confirm('Are you absolutely sure you want to delete your account? This action cannot be undone and will permanently remove all your data.');">
                        @csrf
                        @method('DELETE')
