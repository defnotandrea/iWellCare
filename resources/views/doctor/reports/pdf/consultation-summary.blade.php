...
<body>
    ...
    <div class="section">
        ...
        <div class="chart-container">
            ...
            <div class="chart-bar">
                <div class="chart-bar-fill" style="height: {{ $completedHeight }}px; background-color: #28a745;"></div>
                <div class="chart-bar-value">{{ $completed }}</div>
                <div class="chart-bar-label">Completed</div>
            </div>
            <div class="chart-bar">
                <div class="chart-bar-fill" style="height: {{ $inProgressHeight }}px; background-color: #ffc107;"></div>
                <div class="chart-bar-value">{{ $inProgress }}</div>
                <div class="chart-bar-label">In Progress</div>
            </div>
            <div class="chart-bar">
                <div class="chart-bar-fill" style="height: {{ $pendingHeight }}px; background-color: #6c757d;"></div>
                <div class="chart-bar-value">{{ $total - $completed - $inProgress }}</div>
    ...
    <div class="section">
        ...
        <div style="margin: 20px 0;">
            <div style="margin-bottom: 15px;">
                <strong>Completion Rate:</strong> {{ $completionRate }}% ({{ $completed }} of {{ $total }} consultations)
            </div>
            <div style="margin-bottom: 15px;">
                <strong>In Progress Rate:</strong> {{ $inProgressRate }}% ({{ $inProgress }} of {{ $total }} consultations)
            </div>
            <div style="margin-bottom: 15px;">
                <strong>Pending Rate:</strong> {{ $pendingRate }}% ({{ $total - $completed - $inProgress }} of {{ $total }} consultations)
            </div>
            <div style="margin-bottom: 15px;">
                <strong>Average Duration:</strong> {{ $consultationStats['average_duration'] }}
            </div>
        </div>
    </div>
    ...
    <div class="section">
        ...
        @if($topDoctors->count() > 0)
            <table>
                ...
                <tbody>
                    @foreach($topDoctors as $index => $doctorData)
                    <tr>
                        <td class="text-center">{{ $index + 1 }}</td>
                        <td><strong>{{ $doctorData['doctor']->first_name ?? 'N/A' }}</strong></td>
                        <td class="text-center">{{ $doctorData['count'] }}</td>
                        <td class="text-center">{{ $doctorData['completed'] }}</td>
                        <td class="text-center">{{ $doctorData['in_progress'] }}</td>
                        <td class="text-center">
                            {{ $doctorData['count'] > 0 ? round(($doctorData['completed'] / $doctorData['count']) * 100, 1) : 0 }}%
                        </td>
                    </tr>
                    @endforeach
                </tbody>
