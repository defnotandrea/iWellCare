    ...
    <div class="relative z-20">
            ...
            <div class="border-t-2 border-gray-300 pt-3">
                ...
                <div class="flex justify-center items-center gap-8">
                    <div class="text-left">
                        ...
                        <span class="text-red-600 font-bold text-lg ml-2">{{ $invoiceNumber ?: '0001635' }}</span>
                    </div>
                    <div class="text-left">
                        <span class="font-semibold">DATE:</span>
                        <span class="ml-2">{{ $date ?: date('M d, Y') }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="text" value="{{ $customerName }}" class="w-full p-2 border border-gray-300 rounded bg-gray-50" placeholder="Customer Name">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">TIN:</label>
                <input type="text" value="{{ $customerTin }}" class="w-full p-2 border border-gray-300 rounded bg-gray-50" placeholder="Tax Identification Number">
            </div>
            <div class="col-span-2">
                <label class="block text-sm font-semibold text-gray-700 mb-1">ADDRESS:</label>
                <input type="text" value="{{ $customerAddress }}" class="w-full p-2 border border-gray-300 rounded bg-gray-50" placeholder="Customer Address">
            </div>
            <div class="col-span-2">
                <label class="block text-sm font-semibold text-gray-700 mb-1">SC/PWD/PNSTM/Solo Parent ID No.:</label>
                <input type="text" value="{{ $customerId }}" class="w-full p-2 border border-gray-300 rounded bg-gray-50" placeholder="ID Number">
            </div>
            <div class="col-span-2">
                <label class="block text-sm font-semibold text-gray-700 mb-1">SIGNATURE:</label>
                <input type="text" value="{{ $customerSignature }}" class="w-full p-2 border border-gray-300 rounded bg-gray-50" placeholder="Customer Signature">
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-8 mb-6">
            ...
            <div>
                ...
                <div class="space-y-2">
                    <label class="flex items-center">
                        <input type="radio" name="payment_method" value="cash" {{ $paymentMethod === 'cash' ? 'checked' : '' }} class="mr-2">
                        <span>Cash</span>
                    </label>
                    <label class="flex items-center">
                        <input type="radio" name="payment_method" value="check" {{ $paymentMethod === 'check' ? 'checked' : '' }} class="mr-2">
                        <span>Check</span>
                    </label>
                    <label class="flex items-center">
                        <input type="radio" name="payment_method" value="credit" {{ $paymentMethod === 'credit' ? 'checked' : '' }} class="mr-2">
                        <span>Credit</span>
                    </label>
                </div>

                <!-- Check Details (shown when check is selected) -->
                <div id="check-details" class="mt-4 space-y-2 {{ $paymentMethod === 'check' ? '' : 'hidden' }}">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Check No.:</label>
                        <input type="text" value="{{ $checkNumber }}" class="w-full p-2 border border-gray-300 rounded" placeholder="Check Number">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Date:</label>
                        <input type="date" value="{{ $checkDate }}" class="w-full p-2 border border-gray-300 rounded">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">BANK:</label>
                        <input type="text" value="{{ $bank }}" class="w-full p-2 border border-gray-300 rounded" placeholder="Bank Name">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <input type="number" value="{{ $totalSales }}" class="w-32 p-2 border border-gray-300 rounded text-right" placeholder="0.00">
                </div>
                <div class="flex justify-between items-center">
                    <span class="font-semibold text-gray-700">LESS: SC/PWD/NAAC/SP DISC.:</span>
                    <input type="number" value="{{ $discount }}" class="w-32 p-2 border border-gray-300 rounded text-right" placeholder="0.00">
                </div>
                <div class="flex justify-between items-center">
                    <span class="font-semibold text-gray-700">TOTAL DUE:</span>
                    <input type="number" value="{{ $totalDue }}" class="w-32 p-2 border border-gray-300 rounded text-right" placeholder="0.00">
                </div>
                <div class="flex justify-between items-center">
                    <span class="font-semibold text-gray-700">LESS: WITHHOLDING:</span>
                    <input type="number" value="{{ $withholding }}" class="w-32 p-2 border border-gray-300 rounded text-right" placeholder="0.00">
                </div>
                <div class="flex justify-between items-center border-t-2 border-gray-300 pt-2">
                    ...
                    <div class="flex items-center">
                        ...
                        <input type="number" value="{{ $totalAmountDue }}" class="w-32 p-2 border border-gray-300 rounded text-right font-bold" placeholder="0.00">
                    </div>
                </div>
                <div class="flex justify-between items-center">
                    <span class="font-semibold text-gray-700">SALE/S SUBJ. TO PT. EXEMPT SALES:</span>
                    <input type="number" value="{{ $exemptSales }}" class="w-32 p-2 border border-gray-300 rounded text-right" placeholder="0.00">
                </div>
            </div>
        </div>
        ...
        <div class="mb-6">
            <div class="text-center">
                ...
                <div class="w-64 mx-auto border-b-2 border-gray-400 h-12 flex items-end justify-center">
                    <input type="text" value="{{ $cashierName }}" class="w-full text-center border-0 bg-transparent" placeholder="Signature/Name">
                </div>
            </div>
        </div>
